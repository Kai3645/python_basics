...
if __name__ == "__main__":
	...
	class Application:
		...
		def __init__(self):
			...
			self.frame_1 = tk.Frame(self.win, relief = tk.RAISED,
			                        width = self.FRAME1_WIDTH,
			                        height = self.FRAME1_HEIGHT)
			self.frame_1.grid(row = 0, column = 0, sticky = tk.NW)
			self.frame_2 = tk.Frame(self.win, relief = tk.RAISED,
			...
			B_same = tk.Button(self.frame_2, text = "锁定", width = 2, command = Lock)
			B_same.grid(row = 0, column = 4)
			
			def copy_upper():
				self.E_h_a.delete(0, tk.END)
				self.E_h_a.insert(tk.END, self.E_w_a.get())
				pass
			
			B_same = tk.Button(self.frame_2, text = "复制", width = 2, command = copy_upper)
			B_same.grid(row = 1, column = 4)
			...
			self.can_1 = MainCanvas(self.frame_1, self.FRAME1_WIDTH, self.FRAME1_HEIGHT)
			self.can_1.place(x = 0, y = 0)
			
			# self.can_2 = DisplayCanvas(self.frame_2, self.FRAME2_WIDTH, self.FRAME1_HEIGHT)
